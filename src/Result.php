...
class Result {
  ...
  public function save() {
    if (array_key_exists("id", $this->orig)) {
      ...
      $q = Flight::db()->prepare('UPDATE ' . $this->table . ' SET ' . $columns . ' WHERE id=?');
      ...
      $q = Flight::db()->prepare('INSERT INTO ' . $this->table . ' (' . $columns . ') VALUES (' . $values . ')');
      $q->execute(array_values($data));

      $this->data['id'] = Flight::db()->lastInsertId();
    }
  }
  ...
  public function delete() {
    if (array_key_exists("id", $this->orig)) {
      $q = Flight::db()->prepare('DELETE FROM ' . $this->table . ' WHERE id=?');
